...
<%
	...
	/**检查是否已经报名*/
	UploadWork work = new DaoUtils().selectUploadWork(user, resMatchType.get(index));
	if(work == null)
	{
		response.sendRedirect("error.jsp?error=" + URLEncoder.encode("您还未提交作品，请先提交！", "UTF-8"));
		return;
	}
...
<html>
    ...
    <div class="ui uploadwork modal scrolling">
        <div class="header"><i class="upload icon"></i>修改提交作品</div>
                    ...
                    <form class="ui form" action="./uploadwork.jsp?matchIndex=<%=index%>" method="POST">
                        <div class="fields">
                            <div class="twelve wide field">
                                <label>网盘链接*</label>
                                <input class="dropboxurl" name="dropboxurl" type="text" value="<%=work.getDropboxurl()%>">
                            </div>
                            <div class="four wide field">
                                <label>密码</label>
                                <input class="boxpassword" name="boxpassword" type="text" value="<%=work.getBoxpassword()%>">
                            </div>
                        </div>
